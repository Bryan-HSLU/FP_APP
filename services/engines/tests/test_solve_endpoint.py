...
def test_solve_varianten_details_liefert_k_plaene() -> None:
    ...
    # Kopf = gelieferter Plan (ggf. Begehbarkeits-Leiter, dann NICHT der
    # Score-Sieger); die ÜBRIGEN Karten sind best-first monoton fallend. Ohne
    # Leiter-Massnahme ist damit auch die Gesamtliste monoton.
    tupel = [
        (v["info"]["platziert"], -v["info"]["knapp"], v["info"]["relationScore"]) for v in plaene
    ]
    rest = tupel[1:]
    assert rest == sorted(rest, reverse=True)
    if "begehbarkeit" not in body["varianteInfo"]:
